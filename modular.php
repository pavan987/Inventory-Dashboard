...
$_SESSION['Cookie']=$cookie;
	...
	<inFilter><eq class="computeServerUnit" property="model" value="'.$model.'" /></inFilter>
	</configResolveClass>';
	$rxmlB=curlCall($mxml,$url,$host,$request);


	$computeBlades = simplexml_load_string($rxmlB);
	$computeBlade=$computeBlades->outConfigs->computeServerUnit;
	...
	foreach ($computeBlade as $bladeinfo)
		{
			...
			$availableMemory[$i]=(int)$processorUnits->outConfig->computeBoard->memoryArray['currCapacity'];
